...
final class SiteWebSession: ObservableObject {
    ...
    @Published var lastLoginOpenedAt: Date?
    ...
    func requestLogin() {
        activeFlow = SiteWebFlow(kind: .login, url: baseURL.appendingPathComponent("login"))
        lastLoginOpenedAt = Date()
    }
