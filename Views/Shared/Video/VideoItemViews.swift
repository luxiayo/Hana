...
struct HanaVideoGridCard: View {
    ...
    private func loadWatchedStatus() {
        guard style.showsPlayedIndicator && showPlayedIndicator else {
            isWatched = false
            return
        }
        let allHistory = persistence.loadWatchHistory()
        isWatched = allHistory.first(where: { $0.videoCode == videoCode })?.isWatched == true
    }
