...
private struct LocalDataSettingsScreen: View {
    ...
    @State private var advancedSearchHistoryCount = 0
    @State private var downloadQueueCount = 0
    @State private var hKeyframeRecordsCount = 0
        ...
        Form {
            ...
            Section("数据管理") {
                ...
                LocalDataDeletionButton(
                    ...
                    count: searchHistoryCount + advancedSearchHistoryCount,
    ...
    private func refreshCounts() {
        watchHistoryCount = persistence.loadWatchHistory().count
        searchHistoryCount = persistence.loadSearchHistory().count
        advancedSearchHistoryCount = persistence.loadAdvancedSearchHistory().count
        downloadQueueCount = persistence.loadDownloadQueue().count
        hKeyframeRecordsCount = persistence.loadHKeyframeRecords().count
    }

    private var totalRecordCount: Int {
        watchHistoryCount
            + searchHistoryCount
            + advancedSearchHistoryCount
            + downloadQueueCount
            + hKeyframeRecordsCount
    }
    ...
    private func delete(_ target: LocalDataDeletionTarget) {
        switch target {
        case .watchHistory:
            let records = persistence.loadWatchHistory()
            for record in records {
                persistence.deleteWatchHistory(record)
            }
        case .searchHistory:
            for record in persistence.loadSearchHistory() {
                persistence.deleteSearchHistory(record)
            }
            for record in persistence.loadAdvancedSearchHistory() {
                persistence.deleteAdvancedSearchHistory(record)
            }
        case .downloads:
            deleteDownloadFiles()
            let records = persistence.loadDownloadQueue()
            for record in records {
                persistence.deleteDownloadQueue(record)
            }
        case .hKeyframes:
            let records = persistence.loadHKeyframeRecords()
            for record in records {
                persistence.deleteHKeyframeRecord(record)
            }
        case .all:
            let watchRecords = persistence.loadWatchHistory()
            for record in watchRecords {
                persistence.deleteWatchHistory(record)
            }
            for record in persistence.loadSearchHistory() {
                persistence.deleteSearchHistory(record)
            ...
            deleteDownloadFiles()
            let downloadRecords = persistence.loadDownloadQueue()
            for record in downloadRecords {
                persistence.deleteDownloadQueue(record)
            }
            let hkRecords = persistence.loadHKeyframeRecords()
