    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            .onPreferenceChange(ScrollOffsetKey.self) { offset in
                let newScrollY = max(0, -offset)
                let rounded = (newScrollY * 2).rounded() / 2
                if abs(rounded - scrollY) >= 0.5 {
                    scrollY = rounded
                }
            }
...
private enum HomeHeroForegroundStyle {
    ...
    var titleFont: Font {
        ...
        case .minimal: return .headline.weight(.bold)
        }
    }
    ...
    var showsDescription: Bool { self != .minimal }
    ...
    var descriptionLineLimit: Int {
        ...
        case .compact, .minimal: return 1
        }
    }

    var buttonFont: Font {
        switch self {
        case .regular: return .headline.weight(.semibold)
        case .compact: return .subheadline.weight(.semibold)
        case .minimal: return .caption.weight(.semibold)
        }
    }

    var spacing: CGFloat {
        switch self { case .regular: return 14; case .compact: return 10; case .minimal: return 7 }
    }

    var buttonHorizontalPadding: CGFloat {
        switch self { case .regular: return 17; case .compact: return 14; case .minimal: return 11 }
    }

    var buttonVerticalPadding: CGFloat {
        switch self { case .regular: return 11; case .compact: return 8; case .minimal: return 6 }
    }
}
