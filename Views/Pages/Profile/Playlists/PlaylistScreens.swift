...
private struct PlaylistRow: View {
    let playlist: PlaylistRecordModel
    let count: Int
...
struct PlaylistDetailScreen: View {
    let playlist: PlaylistRecordModel
    @State private var items: [PlaylistItemRecordModel] = []
    ...
    private func loadItems() {
        items = persistence.loadPlaylistItems().filter { $0.playlistID == playlist.id }
            .sorted { $0.createdAt > $1.createdAt }
    }

    private func delete(_ offsets: IndexSet) {
        for index in offsets {
            persistence.deletePlaylistItem(items[index])
        }
